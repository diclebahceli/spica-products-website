...
// This statement has been deleted.
;
export function scheduleCard(config) {
    config = { ...config, method: "get", url: "https://master.spicaengine.com/api/fn-execute/asset/scheduleCard" };
    axiosWriteValidator(config);
    axiosReadValidator(config);
    return axios.request(config).then(r => r.data);
}
export function createScheduleBucket(config) {
    config = { ...config, method: "post", url: "https://master.spicaengine.com/api/fn-execute/asset/createScheduleBucket" };
    axiosWriteValidator(config);
    axiosReadValidator(config);
    return axios.request(config).then(r => r.data);
}
// This statement has been deleted.
;
export function scheduleInsertData(config) {
    config = { ...config, method: "get", url: "https://master.spicaengine.com/api/fn-execute/scheduleInsertData" };
    axiosWriteValidator(config);
    axiosReadValidator(config);
    return axios.request(config).then(r => r.data);
}
