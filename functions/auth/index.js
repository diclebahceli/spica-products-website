...
// This statement has been deleted.
;
export function signUpTrigger(config) {
    config = { ...config, method: "post", url: "https://master.spicaengine.com/api/fn-execute/signup" };
    axiosWriteValidator(config);
    axiosReadValidator(config);
    return axios.request(config).then(r => r.data);
}
export function loginTrigger(config) {
    config = { ...config, method: "post", url: "https://master.spicaengine.com/api/fn-execute/login" };
    axiosWriteValidator(config);
    axiosReadValidator(config);
    return axios.request(config).then(r => r.data);
}
