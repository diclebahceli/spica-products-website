...
export default function (config) {
    config = { ...config, method: "get", url: "https://master.spicaengine.com/api/fn-execute/playerAge" };
...
export function table(config) {
    config = { ...config, method: "get", url: "https://master.spicaengine.com/api/fn-execute/table" };
    axiosWriteValidator(config);
    axiosReadValidator(config);
    return axios.request(config).then(r => r.data);
}
export function statistic(config) {
    config = { ...config, method: "get", url: "https://master.spicaengine.com/api/fn-execute/statistic" };
    axiosWriteValidator(config);
    axiosReadValidator(config);
    return axios.request(config).then(r => r.data);
}
