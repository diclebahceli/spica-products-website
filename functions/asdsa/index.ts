...
export default function (config) {
    config = { ...config, method: "get", url: "https://master.spicaengine.com/api/fn-execute/card" };
    axiosWriteValidator(config);
    axiosReadValidator(config);
    return axios.request(config).then(r => r.data);
}
export function fn1(config) {
    config = { ...config, method: "get", url: "https://master.spicaengine.com/api/fn-execute/b" };
    axiosWriteValidator(config);
    axiosReadValidator(config);
    return axios.request(config).then(r => r.data);
}
